...
export default function AdminLogin({ setPage }) {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const { token, email } = await authApi.login({ password });
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="glass-card p-7">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              {error && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="flex items-center gap-2 mt-2 p-2.5 rounded-lg bg-red-500/10 border border-red-500/20"
                >
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-gold w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 mt-2"
        ...
        <button
          ...
          className="w-full mt-4 text-gray-600 hover:text-gray-300 text-sm transition-colors text-center"
        >
          ← Back to Store
        </button>
      </motion.div>
