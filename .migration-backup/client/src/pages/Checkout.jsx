...
function CopyButton({ text, label }) {
  ...
  return (
    <button onClick={handleCopy} className="flex items-center gap-1 text-yellow-400 hover:text-yellow-300 text-xs transition-colors" data-testid={`button-copy-${label}`}>
...
function CountdownTimer({ seconds }) {
  ...
  return (
    <span className={remaining < 120 ? 'text-red-400' : 'text-yellow-400'}>
      {m}:{s.toString().padStart(2, '0')}
    </span>
  );
}

export default function Checkout({ setPage, setOrderData }) {
  ...
  useEffect(() => {
    if (items.length === 0 && step !== 'success') setPage('cart');
  }, [items]);
  ...
  useEffect(() => {
    if (order && status === 'pending') {
      pollRef.current = setInterval(async () => {
        try {
          const data = await ordersApi.getStatus(order.order_id);
          setStatus(data.payment_status);
          if (data.payment_status === 'confirmed' || data.payment_status === 'failed' || data.payment_status === 'expired') {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        ...
        >
          ...
          <p className="text-yellow-400 font-medium mb-6">{email}</p>
          <p className="text-gray-500 text-sm mb-8">Check your inbox (and spam folder) for your license keys.</p>
          <div className="flex flex-col gap-3">
            <button onClick={() => setPage('products')} className="btn-gold py-3 rounded-xl font-bold" data-testid="button-continue-shopping">
              Continue Shopping
            </button>
            <button onClick={() => setPage('status')} className="text-gray-400 hover:text-white text-sm transition-colors">
              Check Order Status
            </button>
          </div>
        </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                ...
                <div className="glass-card p-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center justify-between bg-black/40 border border-yellow-400/20 rounded-xl p-4">
                        <div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between bg-black/40 border border-yellow-400/20 rounded-xl p-4 gap-3">
                    ...
                    {order.payment.mock && (
                      <div className="border-t border-white/10 pt-4">
                        ...
                        <button
                          ...
                          data-testid="button-mock-confirm"
                        >
              ...
              {(status === 'confirmed' || status === 'failed' || status === 'expired') && (
                <div className="glass-card p-6 text-center">
                  ...
                  {status !== 'confirmed' && (
                    <button onClick={() => setPage('products')} className="btn-gold px-8 py-3 rounded-xl font-bold">
                      Try Again
                    </button>
                  )}
                </div>
        ...
        <div className="glass-card p-5 h-fit">
          ...
          <div className="space-y-3 mb-4">
            {items.map(item => (
              <div key={item.product_id} className="flex gap-2 items-start">
                <div className="w-10 h-10 rounded-lg bg-yellow-900/20 flex items-center justify-center flex-shrink-0">
                ...
                <p className="text-yellow-400 text-xs font-bold flex-shrink-0">${(item.price * item.quantity).toFixed(2)}</p>
              </div>
            ))}
          </div>
          ...
          {email && (
            <div className="mt-3 pt-3 border-t border-white/10">
              ...
              <p className="text-yellow-400 text-xs font-medium break-all">{email}</p>
            </div>
          )}
        </div>
      </div>
