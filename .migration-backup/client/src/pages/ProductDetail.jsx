...
import {
  ShoppingCart, Zap, Package, ChevronLeft, Check, Minus, Plus,
...
export default function ProductDetail({ product: initialProduct, setPage, setOrderData }) {
  ...
  useEffect(() => {
    ...
    productsApi.getById(initialProduct.id).then(setProduct).catch(() => {});
  }, [initialProduct?.id]);
    ...
    <div className="min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16">
          ...
          <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#0d1a0d] to-[#050505] border border-neon-500/15 aspect-square flex items-center justify-center">
            ...
            {!inStock && (
              <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
                <span className="text-red-400 font-bold tracking-widest uppercase border border-red-400/30 px-5 py-2 rounded bg-red-400/10">
                  Out of Stock
                </span>
              </div>
            )}
            ...
            {product.featured === 1 && inStock && (
              <div className="absolute top-4 right-4">
                <span className="tag-green text-xs tracking-wider">★ FEATURED</span>
              </div>
            )}
            ...
            {product.category_name && (
              <div className="absolute top-4 left-4">
                <span className="text-xs px-3 py-1 rounded-full bg-black/60 border border-white/10 text-gray-300">
                  {product.category_name}
                </span>
              </div>
            )}
        ...
        >
          ...
          <div className="glass-card p-5 mb-6 border border-neon-500/20">
            <div className="flex items-end justify-between mb-2">
              <div>
                ...
                <span className="text-4xl font-bold text-neon-500 font-mono">
              ...
              <div className="text-right">
                <span className={`text-sm font-medium px-3 py-1 rounded-full ${
                  inStock
                    ? 'text-neon-500 bg-neon-500/10 border border-neon-500/25'
          ...
          {inStock && (
            <div className="mb-6">
              <label className="text-gray-400 text-sm mb-3 block uppercase tracking-wider text-xs">Quantity</label>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-0">
                  <button
                    onClick={() => setQuantity(q => Math.max(1, q - 1))}
                    disabled={quantity <= 1}
                    className="qty-btn rounded-r-none disabled:opacity-40"
                  >
                    <Minus size={14} />
                  </button>
                  ...
                  <button
                    ...
                    className="qty-btn rounded-l-none disabled:opacity-40"
                  >
                    <Plus size={14} />
                  </button>
                </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <button
              ...
              disabled={!inStock}
              className={`flex-1 flex items-center justify-center gap-2.5 py-4 rounded-xl text-sm font-bold transition-all ${
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2.5 py-4 rounded-xl text-sm font-bold transition-all ${
                inStock
                  ? 'btn-green-solid hover:shadow-[0_0_30px_rgba(0,255,65,0.4)]'
