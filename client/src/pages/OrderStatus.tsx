...
export default function OrderStatus() {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const data: any = await ordersApi.getStatus(orderId.trim());
      setOrder(data);
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="glass-card p-6 mb-6">
          <div className="flex gap-3">
            ...
            />
            <button onClick={handleSearch} disabled={loading} className="btn-gold px-6 py-3 rounded-xl font-bold flex items-center gap-2" data-testid="button-search-order">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="glass-card p-5 space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-neon-500 flex items-center gap-1"><Mail size={13} />{order.customer_email}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="space-y-3">
                {(order.items || []).map((item: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
                    <div className="w-9 h-9 rounded-lg bg-neon-500/10 flex items-center justify-center">
                      <Package size={16} className="text-neon-500" />
                    </div>
                    <div className="flex-1">
                      <p className="text-white text-sm font-medium">{item.name}</p>
                      <p className="text-gray-500 text-xs">Qty: {item.quantity}</p>
                    </div>
                    <p className="text-neon-500 text-sm font-bold">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>
